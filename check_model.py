...
if hasattr(model, "classes_"): print("Classes:", model.classes_)

def proba_human(mdl, Xs):
    if hasattr(mdl, "predict_proba"):
        P = mdl.predict_proba(Xs)
        if hasattr(mdl, "classes_"):
            col = int(np.where(np.array(mdl.classes_) == 1)[0][0])
            return P[:, col]
        return P[:, -1]
...
samples = [
    "I wrote this note after our lab session where the circuit failed inside the enclosure due to heat.",
    "This section provides a concise overview of evaluation methodology comprising ROC-AUC and Brier score.",
...
for t in samples:
    x = scaler.transform([extract_features(t)])
    p = float(proba_human(model, x)[0])
    print(f"{t[:60]}... -> P(human)={p:.3f}")
